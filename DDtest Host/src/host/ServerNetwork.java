...
public class ServerNetwork 
{
	...
	public static void printUptimes(Server[][] servers, int utcTime) //prints single hour
	{
		...
		System.out.println("\nIs all data available? " + dataAvailable);
	}
	
	public static boolean allDataAvailable(Server[][] servers, int utcTime) //prints single hour
	{
		boolean[][] downHosts = new boolean[servers.length][servers[0].length];
		int[] stripeSpeeds = new int[servers.length];
	...
	{
		int[] hostTimezoneArray = new int[120]; //24 hosts, not hours
		Server[][] servers = new Server[4][30]; //4 stripes, 30 mirrors
	...
	public static void testB() //all hosts will be somewhere in North America or Europe (-8:00 to +2:00 UTC)
	{
		int[] hostTimezoneArray = new int[120]; //24 hosts, not hours
		Server[][] servers = new Server[4][30]; //4 stripes, 30 mirrors
		double uptimeVariance = 0.3;
		...
		for (int i = 0; i < hostTimezoneArray.length; i++)
		{
			int randomTime = -8; //PST
			randomTime += (int)(Math.random() * 10); //values: -3, -2, -1, and 0
			hostTimezoneArray[i] = randomTime;
		}
	...
	public static void testB_100k() //all hosts will be somewhere in North America or Europe (-8:00 to +2:00 UTC)
	{
		int[] hostTimezoneArray = new int[120]; //24 hosts, not hours
		Server[][] servers = new Server[4][30]; //4 stripes, 30 mirrors
		double uptimeVariance = 0.3;
		...
		{
			if (!allDataAvailable(servers, num))
				unavailTimes++;
		}
		
		System.out.println("100000 iterations, tiems unavailable: " + unavailTimes);
	}
}
